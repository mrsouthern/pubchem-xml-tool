...
public class CPDPExtractPCAssayFactory {
	...
	public static PubChemAssay getSummaryAssay(Document cpdp){
		...
		addSection("Description:", cpdp, "//ProjectDescription/Paragraph", descriptions, false, false);
		addSection("References:", cpdp, CPDPExtractUtils.getReferenceNodesPath("Text"), descriptions, false, true);
		addSection("Keywords:", cpdp, CPDPExtractUtils.getChosenAIDInfoNodePath(CPDPExtractUtils.KEYWORDS), descriptions, false, false);
	...
	public static PubChemAssay getPubChemAssay(Document cpdp) throws ParseException {
		...
		assay.setGrantNumber(cpdp.selectSingleNode(grantNum).getText());
		assay.setProjectCategory(cpdp.selectSingleNode(CPDPExtractUtils.getChosenAIDInfoNodePath(CPDPExtractUtils.PROJECT_CATEGORY)).getText());
		...
		assay.setExternalRegId(cpdp.selectSingleNode(extRegId).getText());
		...
		assay.setName(cpdp.selectSingleNode(nameAtt).getText());		
		...
		addSection("Assay Overview: ", cpdp, CPDPExtractUtils.getChosenAIDInfoNodePath(CPDPExtractUtils.ASSAY_PURPOSE), protocols, true, false);
		addSection("Protocol Summary: ", cpdp, CPDPExtractUtils.getChosenAIDInfoNodePath(CPDPExtractUtils.PROTOCOL), protocols, false, false);
		CPDPExtractUtils.addNodeTextToList(cpdp, CPDPExtractUtils.getChosenAIDInfoNodePath(CPDPExtractUtils.CALCULATION_SUMMARY), protocols, false);
		addSection("List of Reagents: ", cpdp, CPDPExtractUtils.getChosenAIDInfoNodePath(CPDPExtractUtils.CRITICAL_REAGENTS), protocols, false, false);
		addSection("List of Consumables: ", cpdp, CPDPExtractUtils.getChosenAIDInfoNodePath(CPDPExtractUtils.CONSUMABLES), protocols, false, false);

		assay.setProtocol(protocols);
		
		// comment section
		List<String> comments = new ArrayList<String>();
		CPDPExtractUtils.addNodeTextToList(cpdp, CPDPExtractUtils.getChosenAIDInfoNodePath(CPDPExtractUtils.COMMENTS), comments, false);
	...
	private static void addSection(String section, Document cpdp, String node, List<String> list, boolean isFirst, boolean addListNum) {
		if(! isFirst)
			list.add("");
		list.add(section);
		list.add("");
		CPDPExtractUtils.addNodeTextToList(cpdp, node, list, addListNum);
	}
